...
def turn_light(grid, start, stop, state):
    ...
    for x in range(from_x, to_x + 1):
        for y in range(from_y, to_y + 1):
            if state == TOGGLE:  # increase brightness +2
                grid[y][x] += 2
            elif state == ON:  # increase brightness +1
                grid[y][x] += 1
            else:  # OFF = decrease brightness -1, not less than 0
                grid[y][x] -= 1
                if grid[y][x] < 0:
                    grid[y][x] = 0
...
if __name__ == '__main__':
    ...
    print(f'Day 06, Part 2--Total brightness {total_brightness(grid)}')
